 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_sygc_kcsj_hldjf_v1/lc", desc = "下浮系数服务接口")
// 登录即可访问
...
public class LowerCoefficient extends BaseAction
{
			...
			throws FacadeException
	{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_sygc_kcsj_hldjf_v1/bidOpen/lower.coefficient.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_sygc_kcsj_hldjf_v1/bidOpen/lower.coefficient.tbr.none.html",
					model);
		}
				...
				&& StringUtils.equals("1", confirm.getString("V_STATUS")))
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_sygc_kcsj_hldjf_v1/bidOpen/lower.coefficient.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_sygc_kcsj_hldjf_v1/bidOpen/lower.coefficient.zbr.html",
				model);
	}
