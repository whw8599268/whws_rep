 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_sygc_syjc_hldjf_v1/bspm", desc = "评标基准价计算方法服务")
// 登录即可访问
...
public class BidStandardPriceMethod extends BaseAction
{
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_sygc_syjc_hldjf_v1/bidOpen/bspm.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_sygc_syjc_hldjf_v1/bidOpen/bspm.tbr.none.html",
					model);
		}
		...
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/fjs_sygc_syjc_hldjf_v1/bidOpen/bspm.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/fjs_sygc_syjc_hldjf_v1/bidOpen/bspm.zbr.html", model);
	}
