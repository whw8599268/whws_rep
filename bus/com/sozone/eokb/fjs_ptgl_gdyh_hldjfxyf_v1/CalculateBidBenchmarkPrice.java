 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gdyh_hldjfxyf_v1/cbbp", desc = "计算评标基准价服务")
// 登录即可访问
...
public class CalculateBidBenchmarkPrice extends BaseAction
{
			...
			throws FacadeException
	{
				...
				confirm.getString("V_STATUS"))) && SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/calculate.bsp.tbr.none.html",
					model);
		}
		...
		if (SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/calculate.bsp.tbr.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/calculate.bsp.zbr.html",
				model);
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/calculate.group.bsp.tbr.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/calculate.group.bsp.zbr.html",
				model);
