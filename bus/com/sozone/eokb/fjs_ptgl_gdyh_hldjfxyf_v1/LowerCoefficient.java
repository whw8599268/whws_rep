 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gdyh_hldjfxyf_v1/lc", desc = "下浮系数服务接口")
// 登录即可访问
...
public class LowerCoefficient extends BaseAction
{
			...
			throws FacadeException
	{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.coefficient.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.coefficient.tbr.none.html",
					model);
		}
		// 如果招标人或者代理以及结束了节点
				...
				&& 3 == tpNode.getInteger("N_STATUS"))
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.coefficient.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.coefficient.zbr.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.group.coefficient.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
				...
				&& 3 == tpNode.getInteger("N_STATUS"))
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.group.coefficient.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gdyh_hldjfxyf_v1/secondEnvelope/lower.group.coefficient.zbr.html",
				model);
	}
