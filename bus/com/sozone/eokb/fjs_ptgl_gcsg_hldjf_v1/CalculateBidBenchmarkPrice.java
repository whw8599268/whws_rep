 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gcsg_hldjf_v1/cbbp", desc = "计算评标基准价服务")
// 登录即可访问
...
public class CalculateBidBenchmarkPrice extends BaseAction
{
	 ...
	 * 数据库xml
	 */
	private static String SQLNAME_FIRSTENVELOPE = ConstantEOKB.EOKBBemCode.FJS_PTGL_GCSG_HLDJF_V1
			...
			throws FacadeException
	{
				...
				confirm.getString("V_STATUS"))) && SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/calculate.bsp.tbr.none.html",
					model);
		}

		// 判断是否计算过评标基准价
		List<String> jsons = this.activeRecordDAO.statement().loadList(
				SQLNAME_FIRSTENVELOPE + ".getCBSPInfos", param);
		// 没有计算过
		...
		if (SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/calculate.bsp.tbr.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/calculate.bsp.zbr.html",
				model);
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
			...
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/calculate.group.bsp.tbr.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/calculate.group.bsp.zbr.html",
				model);
			...
			throws ServiceException
	{
		...
		List<String> lcJsons = this.activeRecordDAO.statement().loadList(
				SQLNAME_FIRSTENVELOPE + ".getLCInfos", param);
		// 如果下浮系数为空,说明是第一次进入
