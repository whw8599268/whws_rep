 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gcsg_hldjf_v1/secondenvelope", desc = "第二信封解密服务接口")
// 登录即可访问
...
public class SecondEnvelope extends BaseAction
{
	...
	private static String sqlName = ConstantEOKB.EOKBBemCode.FJS_PTGL_GCSG_HLDJF_V1
			+ "_secondenvelope";
		...
		if (SessionUtils.isBidder())
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollbal.tbr.html",
					model);
		}
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollbal.zbr.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		if (alreadyAllocation)
		{
			...
			if (SessionUtils.isBidder())
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollresult.tbr.html",
						model);

			}
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollresult.zbr.html",
					model);
		}
		...
		if (SessionUtils.isBidder())
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollresult.msg.html",
					model);
		}
		model.put("SECTION_LIST", rollResult);
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/rollresult.html",
				model);
			...
			throws FacadeException
	{
		...
		{
			// 简易招标办法
			if (StringUtils.equals("fjs_ptgl_gcsg_gdsjcqzbr_v1",
					SessionUtils.getTenderProjectTypeCode()))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gcsg_gdsjcqzbr_v1/secondEnvelope/decrypt.zbr.html",
						model);
			}
			// 合理低价法
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/decrypt.zbr.html",
					model);

		}
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/decrypt.tbr.html",
				model);
	}
	...
	public void exclAssets(AeolusData data) throws FacadeException
	{
		...
		try
		{
			input = ClassLoaderUtils
					.getResourceAsStream(
							"com/sozone/eokb/fjs_ptgl_gcsg_hldjf_v1/bidder_info_template.xls",
							this.getClass());
	...
	public void saveSign(AeolusData data) throws ServiceException
	{
		...
		ptgl.createDocFileForGcsgAndGdyh(data, tpid, "fjs_ptgl_gcsg_hldjf_v1",
				"secondRecord");
	}
}
