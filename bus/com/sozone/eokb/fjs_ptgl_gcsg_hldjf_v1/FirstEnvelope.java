 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gcsg_hldjf_v1/firstenvelope", desc = "第一信封解密服务接口")
// 登录即可访问
...
public class FirstEnvelope extends BaseAction
{
		...
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/credit.zbr.html",
					model);
		}
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/credit.tbr.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/review.zbr.html",
					model);
		}
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/review.tbr.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/review.group.zbr.html",
					model);
		}
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstEnvelope/review.group.tbr.html",
				model);
	}
	...
	public ModelAndView getFirstRecord(AeolusData data) throws FacadeException
	{
		...
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/firstenvelope/first.record.view.html",
				model);
	}
