 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_gcsg_hldjf_v1/bspm", desc = "评标基准价计算方法服务")
// 登录即可访问
...
public class BidStandardPriceMethod extends BaseAction
{
			...
			throws FacadeException
	{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.tbr.none.html",
					model);
		}
		// 如果招标人或者代理以及结束了节点
				...
				&& 3 == tpNode.getInteger("N_STATUS"))
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.zbr.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(
						getTheme(data.getHttpServletRequest())
								+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.group.tbr.html",
						model);
			}
			// 未发起确认
			return new ModelAndView(
				...
				&& 3 == tpNode.getInteger("N_STATUS"))
		{
			return new ModelAndView(
					getTheme(data.getHttpServletRequest())
							+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.group.view.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/fjs_ptgl_gcsg_hldjf_v1/secondEnvelope/bspm.group.zbr.html",
				model);
	}
	 ...
	 */
	// 定义路径
	@Path(value = "modify/{tpnid}", desc = "保存标段评标基准价摇号结果")
	// PUT访问方式
