 ...
 * 修改时间：暂无<br/>
 */
package com.sozone.eokb.fjs_gsgl_jdsg_hldjf_v1;
 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_gsgl_jdsg_hldjf_v1/firstenvelope", desc = "第一信封解密服务接口")
// 登录即可访问
...
public class FirstEnvelope extends BaseAction
{
	 ...
	 * 项目CODE
	 */
	private static String projectCode = ConstantEOKB.EOKBBemCode.FJS_GSGL_JDSG_HLDJF_V1;
	 ...
	 */
	// 定义路径
	@Path(value = "/credit/flow/{tpnid}", desc = "获取第一信封解密情况一览表")
	// GET访问方式
	...
	public ModelAndView getFirstCredit(
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("获取投标人信用等级视图", data));
		...
		logger.debug(LogUtils.format("获取投标人信用等级视图完成", data));
			...
			throws FacadeException
	{
		...
		logger.debug(LogUtils.format("修改投标人信用等级完成", data));
	}
	...
	{
		logger.debug(LogUtils.format("获取关联企业信息", data));
		...
		logger.debug(LogUtils.format("获取关联企业信息完成"));
			...
			@PathParam("name") String name) throws FacadeException
	{
		...
		logger.debug(LogUtils.format("搜索企业关联标号完成", data));
		return bidders;
	}
	...
	public void updateCode(AeolusData data) throws FacadeException
	{
		...
		this.activeRecordDAO
				...
				.setParam("name", name).setParam("code", code)
				.setParam("aid", aid).update();
		logger.debug(LogUtils.format("更行关联关系完成"));
	}
			...
			throws FacadeException
	{
		...
		logger.debug(LogUtils.format("查询标段组完成"));
		return sections;
	}
	 ...
	 */
	// 定义路径
	@Path(value = "/electronics/flow/{tpnid}", desc = "电子摇号")
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
	public ModelAndView getFirstElectronics(
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("获取电子摇号视图", data));
		ModelMap model = new ModelMap();
		Record<String, Object> param = new RecordImpl<String, Object>();
		param.setColumn("tpnoid", tenderProjectNodeID);
		Record<String, Object> tpNode = this.activeRecordDAO.auto()
		...
		// 是否发起确认标识
		model.put("alreadyLaunched", alreadyLaunched);
		logger.debug(LogUtils.format("获取电子摇号视图成功"));
		// 招标人
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
	...
	public void doElectronics(AeolusData data) throws FacadeException
	{
		...
		logger.debug(LogUtils.format("电子摇号完成"));
	}
	...
	public ModelAndView getFirstPreview(
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("获取第一信封结果开标结果", data));
		ModelMap model = new ModelMap();
		Record<String, Object> param = new RecordImpl<String, Object>();
		String tpid = SessionUtils.getTPID();
		param.setColumn("tpid", tpid);
			...
			throws FacadeException
	{
		...
		logger.debug(LogUtils.format("成功第一信封结果开标结果"));
		// 招标人
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
