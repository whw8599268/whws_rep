 ...
 * @since 1.0.0
 */
@Path(value = "/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid", desc = "评标前准备服务接口")
// 登录即可访问
...
public class Prebid extends BaseAction
{
	...
	private static String sqlName = ConstantEOKB.EOKBBemCode.XMS_SZGC_SG_JPSDZDTBJZBF_A_V1
			+ "_prebid";
	...
	{
		logger.debug(LogUtils.format("打开抽取K值页面", data));
		...
		logger.debug(LogUtils.format("数据获取完毕，跳转到抽取K值页面"));
	 ...
	 */
	// 定义路径
	@Path(value = "first/{tpnid}", desc = "保存K值摇号结果")
	// PUT访问方式
	@HttpMethod(HttpMethod.POST)
	@Handler(OperationLogHandler.class)
	public void saveKvalue(@PathParam("tpnid") String tenderProjectNodeID,
			AeolusData data) throws FacadeException
	{
		...
		logger.debug(LogUtils.format("成功保存摇号结果"));
	}
	...
	public ModelAndView benchmarkIndex(
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("打开基准价主页", data));
		ModelMap model = new ModelMap();
		String tpid = SessionUtils.getTPID();
		Record<String, Object> param = new RecordImpl<String, Object>();
		param.setColumn("tpnoid", tenderProjectNodeID);
		...
		logger.debug(LogUtils.format("信息获取完成，跳转基准价主页"));
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid/benchmark.index.html",
				model);
	}
	...
	public ModelAndView benchmarkCaculate(@PathParam("sid") String sid,
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("开始基准价计算", data));
		ModelMap model = new ModelMap();
		String tpid = SessionUtils.getTPID();
		Record<String, Object> param = new RecordImpl<String, Object>();
		param.setColumn("tpnoid", tenderProjectNodeID);
		...
		logger.debug(LogUtils.format("基准价计算完成,跳转到基准价页面"));
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid/benchmark.edit.html",
				model);
	}
			...
			AeolusData data) throws FacadeException
	{
		...
		logger.debug(LogUtils.format("成功获取符合搜索条件的入围投标人列表分页封装"));
		return page;
	}

	/**
	 * 
	 * 获取基准价之外和基准价之内的投标人名单<br/>
	 * <p>
	 * 获取基准价之外和基准价之内的投标人名单
	 * </p>
	 * 
	 ...
	 */
	// 定义路径
	@Path(value = "/getBenchamarkList/{sid}/{status}", desc = "获取基准价之外和基准价之内的投标人名单")
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
			...
			throws FacadeException
	{
		...
		for (Record<String, Object> section : sections)
		{
			...
			logger.debug(LogUtils.format(
					"标段" + section.getString("V_BID_SECTION_NAME") + "的方法是",
					method));
		...
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid/preTenderList.index.html",
				model);
	}
			...
			throws FacadeException
	{
		...
		this.activeRecordDAO
				...
				.build(param).update();
		logger.debug(LogUtils.format("成功修改入围状态"));
	}
			...
			throws FacadeException
	{
		...
		logger.debug(LogUtils.format("入围数量：" + n));
		return n;
	}
	...
	{
		logger.debug(LogUtils.format("初始化抽取状态", tpid, tenderProjectNodeID, sid));
		...
		this.activeRecordDAO.auto().table(TableName.TENDER_PROJECT_NODE_DATA)
				.save(record);
		logger.debug(LogUtils.format("成功初始化抽取状态"));
		return record;
	}
	...
	public void modifyExtract(
			...
			AeolusData data) throws FacadeException
	{
		logger.debug(LogUtils.format("开始修改抽取状态", status, data));
		Record<String, Object> param = new RecordImpl<String, Object>();
		String tpid = SessionUtils.getTPID();
		...
		this.activeRecordDAO
				...
				.build(param).update();
		logger.debug(LogUtils.format("成功修改抽取状态", status, data));
	}
			...
			throws FacadeException
	{
		...
		model.put("SECTIONS", sections);
		model.put("TENDER_PROJECT_FLOW_NODE", tpNode);
		logger.debug(LogUtils.format("投标人列表获取成功，跳转到入围投标人页面"));
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid/preTenderList.result.html",
				model);
	}
			...
			AeolusData data) throws FacadeException
	{
		...
		logger.debug(LogUtils.format("成功获取符合搜索条件的入围投标人列表"));
		return page;
	}
			...
			throws FacadeException
	{
		...
		model.put("SECTIONS", sections);
		model.put("TENDER_PROJECT_FLOW_NODE", tpNode);
		logger.debug(LogUtils.format("获取投标人列表完成,跳转到评审的人员名单页面", data));
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_szgc_sg_jpsdzdtbjzbf_A_v1/prebid/reviewTenderList.html",
				model);
	}
