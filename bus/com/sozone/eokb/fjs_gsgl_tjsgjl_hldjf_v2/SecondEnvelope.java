 ...
 * 修改时间：暂无<br/>
 */
package com.sozone.eokb.fjs_gsgl_tjsgjl_hldjf_v2;
 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_gsgl_tjsgjl_hldjf_v2/secondenvelope", desc = "第二信封解密服务接口")
// 登录即可访问
...
public class SecondEnvelope extends BaseAction
{
	 ...
	 * 项目CODE
	 */
	private static String projectCode = ConstantEOKB.EOKBBemCode.FJS_GSGL_TJSGJL_HLDJF_V2;
			...
			throws FacadeException
	{
		...
		logger.debug(LogUtils.format("获取第二信封文件解密结果成功"));
		// 招标人
	...
	public ModelAndView getBenchmarkPriceRecord(AeolusData data)
			throws FacadeException
	{
		logger.debug(LogUtils.format("获取评标基准价记录信息", data));
		ModelMap model = new ModelMap();
		Record<String, Object> param = new RecordImpl<String, Object>();
		String tpid = SessionUtils.getTPID();
		param.put("tpid", tpid);
		...
		model.putAll(GsglUtils.getBenchmarkRecordView(tpid,
				EOKBFlowCode.DEXF_PRICE));
		logger.debug(LogUtils.format("成功获取评标基准价记录信息"));
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/" + projectCode + "/" + EOKBFlowCode.SECOND_ENVELOPE
