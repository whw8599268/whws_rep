 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_kcsj_hldjfxyf_v2/thirdenvelope", desc = "第二信封解密服务接口")
// 登录即可访问
...
public class ThirdEnvelope extends BaseAction
{
	 ...
	 * 日志
	 */
	private static Logger logger = LoggerFactory.getLogger(ThirdEnvelope.class);

	/**
	 * 项目CODE
	 */
	private static String projectCode = ConstantEOKB.EOKBBemCode.FJS_PTGL_KCSJ_HLDJFXYF_V2;
	 ...
	 */
	// 定义路径
	@Path(value = "/thirdRc/flow/{tpnid}", desc = "第三信封文件解密结果")
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
	public ModelAndView getThirdRc(
	...
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
	public ModelAndView getThirdRecord(AeolusData data) throws FacadeException
	{
	...
	public void saveSign(AeolusData data) throws FacadeException
	{
		...
		ptgl.createDocFileForKcHldjf(data, tpid, "thirdRecord", projectCode);
	}
}
