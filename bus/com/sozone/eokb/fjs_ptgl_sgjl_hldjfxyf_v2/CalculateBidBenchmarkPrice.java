/**
 * 包名：com.sozone.eokb.fjs_ptgl_sgjl_hldjfxyf_v2
 ...
 * 修改时间：暂无<br/>
 */
package com.sozone.eokb.fjs_ptgl_sgjl_hldjfxyf_v2;
 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_sgjl_hldjfxyf_v2/cbbp", desc = "计算评标基准价服务")
// 登录即可访问
...
public class CalculateBidBenchmarkPrice extends BaseAction
{
	 ...
	 * 项目CODE
	 */
	private static String projectCode = ConstantEOKB.EOKBBemCode.FJS_PTGL_SGJL_HLDJFXYF_V2;
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
			...
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/" + projectCode + "/"
					+ EOKBFlowCode.SECOND_ENVELOPE + "/calculate.bsp.tbr.html",
					model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
