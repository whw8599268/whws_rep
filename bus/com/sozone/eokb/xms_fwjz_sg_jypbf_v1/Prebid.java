 ...
 * 修改时间：暂无<br/>
 */
package com.sozone.eokb.xms_fwjz_sg_jypbf_v1;
 ...
 * @since 1.0.0
 */
@Path(value = "/xms_fwjz_sg_jypbf_v1/prebid", desc = "评标前准备服务接口")
// 登录即可访问
...
public class Prebid extends BaseAction
{
			...
			throws FacadeException
	{
		...
		for (Record<String, Object> section : sections)
		{
			...
			int method = FjszUtils.chouMethod(tpid, sectionId, "3");
		...
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/xms_fwjz_sg_jypbf_v1/prebid/preTenderList.index.html",
				model);
	}
			...
			throws FacadeException, IOException
	{
		...
		return new ModelAndView(
				getTheme(data.getHttpServletRequest())
						+ "/eokb/xms_fwjz_sg_jypbf_v1/prebid/preTenderList.result.html",
				model);
	}
			...
			throws FacadeException, IOException
	{
		...
		model.put("TENDER_PROJECT_FLOW_NODE", tpNode);
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/xms_fwjz_sg_jypbf_v1/prebid/reviewTenderList.html",
				model);
	}
	 ...
	 */
	// 定义路径
	@Path(value = "/tenderList/flow/{tpnid}", desc = "打开入围投标人页面")
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
	public ModelAndView tenderListIndex(
			@PathParam("tpnid") String tenderProjectNodeID, AeolusData data)
			throws FacadeException
	{
		logger.debug(LogUtils.format("打开入围投标人页面", data));
		...
		int count = this.activeRecordDAO
				...
				.count(param);
		model.put("HAS_ALGORITHM", count > 0);
		model.put("TENDER_PROJECT_FLOW_NODE", tpNode);
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/xms_fwjz_sg_jypbf_v1/prebid/tenderList.html", model);
	}
