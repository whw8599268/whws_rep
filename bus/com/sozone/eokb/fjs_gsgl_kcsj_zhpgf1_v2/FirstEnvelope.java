 ...
 * 修改时间：暂无<br/>
 */
package com.sozone.eokb.fjs_gsgl_kcsj_zhpgf1_v2;
 ...
 * @since 1.0.0
 */
@Path(value = "/fjs_gsgl_kcsj_zhpgf1_v2/firstenvelope", desc = "第一信封解密服务接口")
// 登录即可访问
...
public class FirstEnvelope extends BaseAction
{
	 ...
	 * 项目CODE
	 */
	private static String projectCode = ConstantEOKB.EOKBBemCode.FJS_GSGL_KCSJ_ZHPGF1_V2;
			...
			for (Record<String, Object> section : sections)
			{
				...
				for (Record<String, Object> bidder : bidders)
				{
					...
					this.activeRecordDAO.auto()
							.table(TableName.EKB_T_TENDER_LIST).modify(temp);
				}
	...
	public void beginReview() throws FacadeException
	{
		...
		this.activeRecordDAO.auto().table(TableName.EKB_T_SECTION_INFO)
				...
				.setCondition("AND", "V_BID_OPEN_STATUS NOT LIKE '10%'")
				.modify(param);
		logger.debug(LogUtils.format("修改第一信封评审结果成功"));
	}
