 ...
 * 第二信封解密服务接口<br/>
 * </p>
 * Time：2017-10-10 下午2:13:39<br/>
 * 
 * @author wengdm
 * @version 1.0.0
 * @since 1.0.0
 */
@Path(value = "/fjs_ptgl_kcsj_zhpgf1_v2/secondenvelope", desc = "第一信封解密服务接口")
// 登录即可访问
...
public class SecondEnvelope extends BaseAction
{
	 ...
	 */
	// 定义路径
	@Path(value = "/review/flow/{tpnid}", desc = "第二信封评审结果")
	// GET访问方式
	...
	public ModelAndView getFirstReview(
			...
			throws FacadeException
	{
		logger.debug(LogUtils.format("第二信封结果开标结果", data));
		...
		if (!StringUtils.equals("0", status)
				&& !StringUtils.equals("2-2", status))
		{
			psover = "1";
			model.putAll(PtglUtils.getReviewRecordForKc(tpid,"2-2"));
		}
		...
		if (!SessionUtils.isBidder())
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					...
					+ "/secondReview.zbr.html", model);
		}
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/" + projectCode + "/" + secondEnvelope
				+ "/secondReview.tbr.html", model);
	}
