...
public class BidBenchmarkPriceAlgorithmUtils
{
	 ...
	 * @return 结果
	 */
	public static Record<String, Object> canUseMethodTwoForGDYH(
			List<Double> prices, Double maxPrice)
	{
		Record<String, Object> result = new RecordImpl<String, Object>();
		result.setColumn("ADAPTE", true);
		// 不存在有效投标报价
		if (CollectionUtils.isEmpty(prices))
		{
			result.setColumn("MEMO", "不存在有效评标价!");
			return result;
		}
		// 四舍五入值,计算最高限价95%
		Long mp1 = Math.round(maxPrice * 0.95);
		...
		if (0 == count)
		{
			result.setColumn("MEMO", "不存在小于最高限价的95%且大于最高限价的90%的评标价!");
			return result;
		}
		result.setColumn("MEMO", "小于最高限价的95%且大于最高限价的90%的评标价共" + count + "家!");
		return result;
	}
	 ...
	 * @return 结果
	 */
	public static Record<String, Object> canUseMethodOneForGDYH(
			List<Double> prices, Double maxPrice)
	{
		Record<String, Object> result = new RecordImpl<String, Object>();
		result.setColumn("ADAPTE", true);
		...
		if (0 == count)
		{
			result.setColumn("MEMO", "不存在大于等于最高限价的95%的评标价!");
			return result;
		}
		result.setColumn("MEMO", "大于等于最高限价的95%的评标价共" + count + "家!");
		return result;
	}
			...
			Double coefficient, int algorithm) throws ServiceException
	{
		...
		if (BBPAlgorithm.MethodOne == mt)
		{
			if (StringUtils.contains(SessionUtils.getTenderProjectTypeCode(),
					"gdyh"))
			{
				return methodOneForGDYH(prices, maxPrice, coefficient);
			}
		...
		if (BBPAlgorithm.MethodTwo == mt)
		{
			if (StringUtils.contains(SessionUtils.getTenderProjectTypeCode(),
					"gdyh"))
			{
				return methodTwoForGDYH(prices, maxPrice, coefficient);
			}
	 ...
	 *             服务异常
	 */
	public static Long methodOneForGDYH(List<Double> prices, Double maxPrice,
			Double coefficient) throws ServiceException
	{
		// 先校验是否可以执行该方法进行计算
		// 判断下浮系数是否在2～5%
		...
		if (CollectionUtils.isEmpty(effectivePrices))
		{
			// 所有报价均不在此区间，则使用最高限价下浮
			Double result = mp - mp * coefficient;
			// 四舍五入取整
			return Math.round(result);
	 ...
	 *             服务异常
	 */
	public static Long methodTwoForGDYH(List<Double> prices, Double maxPrice,
			Double coefficient) throws ServiceException
	{
		// 先校验是否可以执行该方法进行计算
		// 判断下浮系数是否在-1～2%
		...
		for (Double price : prices)
		{
			...
			// 小于最高限价的95%且大于最高限价的90%的有效投标报价
			if (price < Math.round(maxPrice * 0.95)
					&& price > Math.round(maxPrice * 0.9))
			{
				effectivePrices.add(price);
			}
		}
		...
		{
			// 有效报价算术平均值
			Double result = maxPrice - maxPrice * coefficient;
			// 四舍五入取整
			return Math.round(result);
		}
		// 有效报价算术平均值
			...
			Double coefficient) throws ServiceException
	{
		...
		// 2018-06-07 wengdm
		// 由于暂列金问题，导致抽取下浮系数和评标基准价时。两处下浮区间不一致，估在评标基准价计算的时候去掉区间的验证，以实际抽取的下浮系数为准
		if (1 == 1)
		{
			return ltfm(avg, coefficient);
		}
	 ...
	 * @throws ServiceException
	 */
	private static long ltfm(long avg, Double coefficient)
			throws ServiceException
	{
		Double result = avg - avg * coefficient;
		// 四舍五入取整
