 ...
 * @since 1.0.0
 */
@Path(value = "/ptc/cbbp", desc = "计算评标基准价服务")
// 登录即可访问
...
public class CalculateBidBenchmarkPrice extends BaseAction
{
			...
			throws FacadeException
	{
				...
				confirm.getString("V_STATUS"))) && SessionUtils.isBidder())
		{
			...
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/calculate.bsp.tbr.none.html", model);
		}

		// 判断是否计算过评标基准价
		List<String> jsons = this.activeRecordDAO.statement().loadList(
				mybatisName + ".getCBSPInfos", param);
		// 没有计算过
		...
		// 基准价视图
		model.putAll(PtglUtils.getCalculateBidBenchmarkPriceView(jsons, tpid));
		...
		if (SessionUtils.isBidder())
		{
			// 如果已经发起确认
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/calculate.bsp.tbr.html", model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/bus/common/calculate.bsp.zbr.html", model);
	 ...
	 */
	// 定义路径
	@Path(value = "viewgroup/{tpnid}", desc = "打开评标基准价计算结果页面")
	// GET访问方式
	@HttpMethod(HttpMethod.GET)
			...
			throws FacadeException
	{
		...
		model.putAll(PtglUtils.getCalculateBidBenchmarkPriceGroupView(jsons,
				tpid));
		...
		if (SessionUtils.isBidder())
		{
			// 如果已经发起确认
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/calculate.group.bsp.tbr.html", model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/bus/common/calculate.group.bsp.zbr.html", model);
			...
			throws ServiceException
	{
		...
		for (String json : lcJsons)
		{
			...
			List<Double> prices = getDoubleList(ps, temporaryValuation,
					provisionalMmoney);
			...
			long bbp = BidBenchmarkPriceAlgorithmUtils.calculate(prices,
					section.getDouble("N_CONTROL_PRICE") - temporaryValuation
							- provisionalMmoney, coe,
			...
			Double temporaryValuation, Double provisionalMmoney)
	{
		...
		{
			rs.add(Arith.sub(Arith.sub(array.getDouble(i), temporaryValuation),
					provisionalMmoney));
		}
		return rs;
	}
	...
	public String exclAssets(AeolusData data) throws FacadeException
	{
		...
		List<Record<String, Object>> bidders = this.activeRecordDAO.statement()
				.selectList(mybatisName + ".getExcelBidderInfo", param);
