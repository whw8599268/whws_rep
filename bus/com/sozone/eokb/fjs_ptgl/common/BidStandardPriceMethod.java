 ...
 * @since 1.0.0
 */
@Path(value = "/ptc/bspm", desc = "评标基准价计算方法服务")
// 登录即可访问
...
public class BidStandardPriceMethod extends BaseAction
{
			...
			throws FacadeException
	{
		...
		{
			// 2018-09-30 国道掩护抽取评标办法变更，去除小于15家和方法三
			if (StringUtils.contains(SessionUtils.getTenderProjectTypeCode(),
					"gdyh"))
			{
				bspmJsons = initBSPMDataForGDYH(tpid, tenderProjectNodeID);
			}
			else
			{
		...
		if (SessionUtils.isBidder())
		{
					...
					&& StringUtils.equals("1", confirm.getString("V_STATUS")))
			{
				return new ModelAndView(getTheme(data.getHttpServletRequest())
						+ "/eokb/bus/common/bspm.tbr.html", model);
			}
			// 未发起确认
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/bspm.tbr.none.html", model);
		}
		// 如果招标人或者代理以及结束了节点
		...
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/bspm.view.html", model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/bus/common/bspm.zbr.html", model);
	}
			...
			throws ServiceException
	{
		...
		de.select("ID", "V_BID_SECTION_ID", "V_BID_SECTION_CODE",
				...
				"N_PROJECT_PROVISIONAL_MONEY", "N_PROJECT_TEMPORARY_VALUATION")
		...
		for (Record<String, Object> section : sections)
		{
			...
			prices = this.activeRecordDAO.statement().loadList(
					mybatisName + ".getSectionEffectivePrice", param);
	...
	private List<String> initBSPMDataForGDYH(String tpid,
			String tenderProjectNodeID) throws ServiceException
	{
		...
		for (Record<String, Object> section : sections)
		{
			...
			// 暂估价
			provisionalMoney = section.getDouble("N_PROJECT_PROVISIONAL_MONEY");
			if (null == provisionalMoney)
			{
				provisionalMoney = 0D;
			}
			// 暂列金
			...
			// 方法一
			bspm.setColumn("METHOD_ONE", BidBenchmarkPriceAlgorithmUtils
					.canUseMethodOneForGDYH(effectivePrices, maxPrice));
			// 方法二
			bspm.setColumn("METHOD_TWO", BidBenchmarkPriceAlgorithmUtils
					.canUseMethodTwoForGDYH(effectivePrices, maxPrice));

			// 方法三（不需要，为了不报错）
			bspm.setColumn("METHOD_THREE", new RecordImpl<String, Object>().setColumn("ADAPTE", false));
			json = JSON.toJSONString(bspm);
			record.setColumn("V_JSON_OBJ", json);
			this.activeRecordDAO.auto()
					.table(TableName.TENDER_PROJECT_NODE_DATA).save(record);
			...
			throws FacadeException
	{
		...
		if (SessionUtils.isBidder())
		{
			...
			{
				return new ModelAndView(getTheme(data.getHttpServletRequest())
						+ "/eokb/bus/common/bspm.group.tbr.html", model);
			}
			// 未发起确认
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/bspm.tbr.none.html", model);
		...
		{
			return new ModelAndView(getTheme(data.getHttpServletRequest())
					+ "/eokb/bus/common/bspm.group.view.html", model);
		}
		// 如果招标人或者招标代理
		return new ModelAndView(getTheme(data.getHttpServletRequest())
				+ "/eokb/bus/common/bspm.group.zbr.html", model);
	}
	 ...
	 * 
	 * <P>
	 * <font color="red">新增暂列金和暂估价，如果这两个有值，计算评标基准价的投标人报价需要减去这两个价格(取消了，
	 * 只在计算的时候体现新增暂列金和暂估价)</font>
	 * </p>
	 * 
	 * @param prices
	 *            有效价格列表
	 ...
	 * @param minPrice
	 *            最低限价
	 * @param temporaryValuation
	 *            暂估价
	 * @param provisionalMmoney
	 *            暂列金
	 * @return
	 */
	private static List<Double> getEffectivePriceForGDYH(List<Double> prices,
			Double maxPrice, Double minPrice, Double temporaryValuation,
			Double provisionalMmoney)
	{
		if (CollectionUtils.isEmpty(prices))
		{
			return prices;
		...
		for (Double price : prices)
		{
			if (null != price && price <= maxPrice && price >= minPrice)
			{
				rs.add(Arith.sub(Arith.sub(price, temporaryValuation),
						provisionalMmoney));
			}
		}
		return rs;
