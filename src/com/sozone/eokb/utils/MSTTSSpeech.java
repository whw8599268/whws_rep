 ...
 * 文件名：MSTTSSpeech.java<br/>
 * 创建时间：2018-1-7 上午11:35:27<br/>
 * 创建者：LDH<br/>
 * 修改者：暂无<br/>
 * 修改简述：暂无<br/>
 * 修改详述：
...
public class MSTTSSpeech
{
	 ...
	 *            语音库序号
	 */
	public void changeVoice(int voi)
	{
		if (this.voice != voi)
		{
			this.voice = voi;
		...
		try
		{
			Dispatch voiceItems = Dispatch.call(spVoice, "GetVoices")
					.toDispatch();
			int count = Integer.valueOf(Dispatch.call(voiceItems, "Count")
					.toString());
			if (count > 0)
			{
				Dispatch voiceItem = Dispatch.call(voiceItems, "Item",
						new Variant(this.voice)).toDispatch();
	...
	public void changeAudioOutput(int audio)
	{
		...
		try
		{
			Dispatch audioOutputs = Dispatch.call(spVoice, "GetAudioOutputs")
					.toDispatch();
			int count = Integer.valueOf(Dispatch.call(audioOutputs, "Count")
					.toString());
			if (count > 0)
			{
				Dispatch audioOutput = Dispatch.call(audioOutputs, "Item",
						new Variant(this.audio)).toDispatch();
		...
		try
		{
			...
			if (count > 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					String voice = Dispatch.call(voiceItem, "GetDescription")
							.toString();
					voices[i] = voice;
				}
			}
	...
	public String[] getAudioOutputs()
	{
		...
		try
		{
			...
			if (count > 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					Dispatch voiceItem = Dispatch.call(audioOutputs, "Item",
							new Variant(i)).toDispatch();
					String voice = Dispatch.call(voiceItem, "GetDescription")
							.toString();
					result[i] = voice;
	...
	public static void main(String[] args)
	{
		...
		speech.speak(text);
		// speech.setRate(-1);
		// speech.saveToWav(text, "E://test.mp3");
